import { getUserByEmail , createUser, comparePassword , sendVerifyMail } from "./authService";
...
export const regCont = async(req:any , res:any)=>{
    try {
        ...
        // save user
        const user = await createUser({lastName,firstName,email,password:hashedPassword});
        res.send({
            success:true,
            user
        });
...
export const loginCont = async(req:any , res:any)=>{
    ...
    // JWT sign
    const token = await JWT.sign({id:user._id},process.env.JWT_SECRET,{ expiresIn: "4000s" });
    if(user){
